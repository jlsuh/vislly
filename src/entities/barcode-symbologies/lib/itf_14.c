...
static inline int draw_start_pattern(Canvas *c, int x, int y, int n_bar, int n_space, int height)
{
    int offset = 0;
...
static inline int draw_stop_pattern(Canvas *c, int x, int y, int n_bar, int w_bar, int n_space, int height)
{
    int offset = 0;
...
static inline int draw_interleaved_2_of_5(Canvas *c, int x, int y, int n_bar, int w_bar, int n_space, int w_space,
                                          int height, size_t group_start_index, size_t group_end_index)
{
    int offset = 0;
    ...
    for (size_t i = group_start_index; i < group_end_index; i += 2) {
        ...
        for (size_t j = 0; j < ITF14_WIDTHS_PER_DIGIT; ++j) {
            offset += draw_pattern(c, ITF14_BINARY_BAR, x + offset, y,
...
void render(void)
{
    ...
    curr_x += draw_start_pattern(&c, curr_x, curr_y, n_bar, n_space, bar_height_px);
    curr_x += draw_interleaved_2_of_5(&c, curr_x, curr_y, n_bar, w_bar, n_space, w_space, bar_height_px,
                                      ITF14_START_INDEX, ITF14_CHECKSUM_INDEX);
    curr_x += draw_stop_pattern(&c, curr_x, curr_y, n_bar, w_bar, n_space, bar_height_px);
}
