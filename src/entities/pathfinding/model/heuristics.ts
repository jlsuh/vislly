...
type HeuristicsName = (typeof HeuristicsName)[keyof typeof HeuristicsName];

const HeuristicsNames: ReadonlyDeep<HeuristicsName[]> =
  Object.values(HeuristicsName);
...
const Heuristics: ReadonlyDeep<Record<HeuristicsName, HeuristicsFunction>> = {
  [HeuristicsName.Chebyshev]: (from: Vertex, to: Vertex): number => {
    return Math.max(Math.abs(to.row - from.row), Math.abs(to.col - from.col));
  },
  [HeuristicsName.Diagonal]: (from: Vertex, to: Vertex): number => {
  ...
  [HeuristicsName.Euclidean]: (from: Vertex, to: Vertex): number => {
    return Math.sqrt((to.row - from.row) ** 2 + (to.col - from.col) ** 2);
  },
  [HeuristicsName.Manhattan]: (from: Vertex, to: Vertex): number => {
    return Math.abs(to.row - from.row) + Math.abs(to.col - from.col);
  },
  [HeuristicsName.Octile]: (from: Vertex, to: Vertex): number => {
    const dx = Math.abs(to.row - from.row);
    const dy = Math.abs(to.col - from.col);
...
const INITIAL_HEURISTICS: HeuristicsName = HeuristicsName.Manhattan;
